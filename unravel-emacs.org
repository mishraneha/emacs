...
- [[#the-unravel-themeel-module][The =unravel-theme.el= module]]
  ...
  - [[#the-unravel-themeel-section-for-my-minimal-highlight-themes-alabaster-themes][The =unravel-theme.el= section for my minimal highlight themes (~alabaster-themes~)]]
...
- [[#the-unravel-essentialsel-module][The =unravel-essentials.el= module]]
  ...
  - [[#the-unravel-essentialsel-section-for-code-folding-based-on-tree-sitter-treesit-fold][The =unravel-essentials.el= section for code-folding based on tree-sitter (~treesit-fold~)]]
...
- [[#the-unravel-completionel-module][The =unravel-completion.el= module]]
  ...
  - [[#the-unravel-completionel-section-for-tagref-completions-tagrefel][The =unravel-completion.el= section for tagref completions (~tagref.el~)]]
...
- [[#the-unravel-searchel-module][The =unravel-search.el= module]]
  ...
  - [[#the-unravel-searchel-settings-for-semantic-searching-in-programming-ast-grep][The =unravel-search.el= settings for semantic searching in programming (~ast-grep~)]]
...
- [[#the-unravel-windowel-module][The =unravel-window.el= module]]
  ...
  - [[#the-unravel-windowel-use-of-contextual-header-line-breadcrumb][The =unravel-window.el= use of contextual header line (~breadcrumb~)]]
...
- [[#the-unravel-orgel-module][The =unravel-org.el= module]]
  ...
  - [[#the-unravel-orgel-section-on-cliplink-org-cliplink][The =unravel-org.el= section on Cliplink (~org-cliplink~)]]
...
- [[#the-unravel-langsel-module][The =unravel-langs.el= module]]
  ...
  - [[#the-unravel-langsel-section-for-ai-assisted-coding][The =unravel-langs.el= section for AI Assisted Coding]]
  ...
  - [[#the-unravel-langsel-section-for-yaml-and-toml][The =unravel-langs.el= section for YAML and TOML]]
  - [[#the-unravel-langsel-section-for-golang][The =unravel-langs.el= section for Golang]]
  - [[#the-unravel-langsel-section-for-dafny][The =unravel-langs.el= section for Dafny]]
...
- [[#the-unravel-studyel-module][The =unravel-study.el= module]]
  ...
  - [[#the-unravel-studyel-section-for-focused-work-org-pomodoro-third-time-and-third-time][The =unravel-study.el= section for focused work (~org-pomodoro-third-time~ and ~third-time~)]]
...
I use themes from the ~ef-themes~ package when I want "cool" themes.
...
** The =unravel-theme.el= section for my minimal highlight themes (~alabaster-themes~)
:PROPERTIES:
:CUSTOM_ID: h:E259D8CD-6FE7-43A2-A8A6-FFE3872CAC27
:CREATED:  [2025-12-21 Sun 12:48]
:END:

I fell in love with Tonsky's Alabaster Theme philosophy and built an
equivalent for Emacs. These are my go-to themes now.

Refer to the README file of the ~alabaster-themes~ package for more details.

I am the lead developer and maintainer of this package, it is inspired
directly from Tonsky's Sublime Alabaster theme and from Prot's
~ef-themes~ package.

+ Package name (GNU ELPA): ~alabaster-themes~
+ Change log: <https://github.com/vedang/alabaster-themes/blob/main/CHANGELOG.org>
+ Git repositories:
  - GitHub: <https://github.com/vedang/alabaster-themes>
...
We use [[https://typeof.net/Iosevka/][Iosveka]] font in the setup, please install the font in your system.
You can also choose to change the family to pick your preferred font.
...
** The =unravel-essentials.el= section for code-folding based on tree-sitter (~treesit-fold~)
:PROPERTIES:
:CUSTOM_ID: h:DB276067-F913-4E4D-A1C6-73A5A99EBEC1
:CREATED:  [2025-10-13 Mon 21:21]
:END:
I took this package from watching BG program. I expect that it will eventually completely replace my use of ~outline-minor-mode~
#+begin_src emacs-lisp :tangle "unravel-modules/unravel-essentials.el"
...
*** Attempts to get back ~helm-resume~ functionality (~vertico-repeat~, ~vertico-suspend~)
:PROPERTIES:
:CUSTOM_ID: h:28B5DE20-67DC-40A6-992C-0883BF0CAF2C
:CREATED:  [2025-06-08 Sun 11:53]
:END:
I use ~vertico-repeat~ and ~vertico-suspend~ to mimic the functionality that ~helm-resume~ would provide. Neither of them work well, and this is a poor substitute. Even so, beggars choosers etc.
...
#+end_src
** The =unravel-completion.el= section for tagref completions (~tagref.el~)
:PROPERTIES:
:CUSTOM_ID: h:A4C07D86-58EC-44BB-B8DE-52AF86A7C598
:CREATED:  [2026-01-08 Thu 21:52]
:END:

Stephan Boyer's tagref is a go-to tool for me in all the professional
code I write. It's a small but incredibly elegant tool for writing
clean documentation.

I wrote ~tagref.el~ to make it easy to use tagref from inside Emacs.
Refer to the README file of the ~tagref.el~ package for more details.

+ Package name: ~tagref.el~
+ Git repositories:
  - GitHub: <https://github.com/vedang/tagref.el>
...
** The =unravel-search.el= settings for semantic searching in programming (~ast-grep~)
:PROPERTIES:
:CUSTOM_ID: h:B6B2ADF8-6E17-4F52-A37C-4298899620F3
:CREATED:  [2025-09-19 Fri 11:23]
:END:

AST-Grep is amazing for complex refactoring and code transformations.
More details at:
- Main page: https://ast-grep.github.io/
- Playground: https://ast-grep.github.io/playground.html
- MCP server: https://github.com/ast-grep/ast-grep-mcp
...
~breadcrumb~ is an interesting package when demo'ing something, but not useful to me in day-to-day use.
...
** The =unravel-org.el= section on Cliplink (~org-cliplink~)
:PROPERTIES:
:CUSTOM_ID: h:EA27A5E2-9243-42DE-8B96-AB1DD2021368
:CREATED:  [2025-06-08 Sun 12:24]
:END:
~org-cliplink~ is a small and convenient library for copying links to
various websites into org-mode. It takes the link from the clipboard
and converts it to Org-mode syntax (with the link/description)
...
I use ~eglot~ as the main LSP entry point, and as such, I have
key-bindings for the common functionality implemented by ~eglot~.

Note that demanding ~eglot~ is not a mistake. I want it loaded so that
I can advice functions for different programming langauges. For
example, Python uses ~emacs-pet~ for tracking the environment of each
project, and this needs to load after ~eglot~ so that it can advice
...
** The =unravel-langs.el= section for AI Assisted Coding
:PROPERTIES:
:CUSTOM_ID: h:3B793A7F-C085-4DAC-A594-06F0CEF09936
:CREATED:  [2025-12-05 Fri 09:20]
:END:
*** The =unravel-langs.el= section for aider (~aider.el~)
:PROPERTIES:
:CUSTOM_ID: h:890A51B7-F064-415B-8E20-C21525A7E420
:CREATED:  [2025-07-13 Sun 11:17]
:END:
I love =aider= and have been using it for ages. While my preference is
still to directly use Aider with [[https://aider.chat/docs/usage/watch.html][AI Commands]], I am experimenting with
=aider.el= to see if the embedded prompts add value to my coding
workflow.
...
*** The =unravel-langs.el= section for Claude Code (~claude-code-ide.el~)
:PROPERTIES:
:CUSTOM_ID: h:252F290B-5D4C-48FD-875B-279E274E383E
:CREATED:  [2025-09-09 Tue 21:43]
:END:
I'm not a big fan of Claude Code, but the world is converging on it.
And I see a lot of value in having common conventions for things. So
if these are Claude Code conventions, so be it.
...
*** The =unravel-langs.el= section for generic LLM stuff in Emacs (~gptel~)
:PROPERTIES:
:CUSTOM_ID: h:1D48C505-CBBD-43C3-84BB-81AFB7AB46D4
:CREATED:  [2025-12-05 Fri 09:22]
:END:
...
*** The =unravel-langs.el= section for AI Code Interface (~ai-code-interface.el~)
:PROPERTIES:
:CUSTOM_ID: h:BF9DD23B-8B26-4ACD-867A-ED977F9DD57F
:CREATED:  [2025-12-03 Wed 18:20]
:END:
...
- =basedpyright= as the language server
...
- =uv add ruff basedpyright --group dev=
  ...
  + BasedPyright (provides the binary ~basedpyright~) is a popular fork of the Python Language Server created by Microsoft.
...
*** The =unravel-langs.el= section for Jupyter
:PROPERTIES:
:CUSTOM_ID: h:00A4014C-FB97-44E0-B051-2B3235147FAF
:CREATED:  [2025-09-09 Tue 22:01]
:END:
Note TBD
...
** The =unravel-langs.el= section for YAML and TOML
:PROPERTIES:
:CUSTOM_ID: h:4C84A0B9-DBA3-4016-B40D-22A6C98E1A04
:CREATED:  [2025-07-14 Mon 09:12]
:END:
YAML is a terrible, terrible markup language. But, as is always the
case in this cruel world, it's something we absolutely cannot live
without. Luckily, Emacs 30+ has treesitter support for YAML, so the
experience is much smoother than before.

The same can be said for TOML (mostly). Though it's a bit nicer than YAML.

~taplo-cli~ is a cargo-based LSP server for TOML. Start it with: ~taplo lsp stdio~
#+begin_src emacs-lisp :tangle "unravel-modules/unravel-langs.el"
...
** The =unravel-langs.el= section for Golang
:PROPERTIES:
:CUSTOM_ID: h:CF89AF93-C094-42E1-A836-A1A6B6B233C1
:CREATED:  [2025-10-07 Tue 14:30]
:END:
#+begin_src emacs-lisp :tangle "unravel-modules/unravel-langs.el"
...
** The =unravel-langs.el= section for Dafny
:PROPERTIES:
:CUSTOM_ID: h:9D7C4D64-A8B4-4D18-A89E-6F1B2D3A3F5A
:CREATED:  [2026-01-13 Tue 18:32]
:END:

Dafny ships a language server. With newer Dafny releases, the CLI
provides =\"dafny server\"= (make sure the =dafny= executable is on
your PATH).
...
#+begin_src emacs-lisp :tangle "unravel-modules/unravel-study.el" :mkdirp yes
  ...
  \n"))
#+end_src
*** The =unravel-study.el= denote section for silos  (~denote-silo~)
:PROPERTIES:
:CUSTOM_ID: h:F4F20B9B-16DA-487A-AB30-8D750936EB23
:CREATED:  [2025-06-08 Sun 15:33]
:END:
...
*** The =unravel-study.el= denote section for working with org  (~denote-org~)
*** The =unravel-study.el= denote section for journaling  (~denote-journal~)
:PROPERTIES:
:CUSTOM_ID: h:633DD744-E4A6-455B-80BF-823AA78EDA95
:CREATED:  [2025-06-08 Sun 15:12]
:END:
...
*** The =unravel-study.el= section for timers (~tmr~)
I use timers very rarely, but they are really nifty for when I want them. Mainly, I find myself using timers with ~denote-journal~ and with ~org-tree-slide~. (see: [[#h:B6449CD3-2D29-46EB-8366-7E4E70EA71CE][My personal settings for Emacs-based presentations (~org-tree-slide~)]])
...
*** The =unravel-study.el= denote section on signatures (~denote-sequence~)
:PROPERTIES:
:CUSTOM_ID: h:8514E702-BAC9-400F-A462-D87CE17EB912
:CREATED:  [2025-06-08 Sun 14:38]
:END:
...
** The =unravel-study.el= section for focused work (~org-pomodoro-third-time~ and ~third-time~)
:PROPERTIES:
:CUSTOM_ID: h:B5A1252F-0BBC-4028-90E3-168EBFDF8835
:CREATED:  [2025-09-09 Tue 22:00]
:END:
~org-pomodoro-third-time~ is my favorite way of using the Pomodoro
technique. Read more about it here:
https://www.lesswrong.com/posts/RWu8eZqbwgB9zaerh/third-time-a-better-way-to-work

I'm also experimenting with =~swflint/third-time=, which is a simple
way to track work-time without having to clock into a task in
org-mode. This doesn't fit smoothly into my workflow yet, but it's so
small and convenient that I'm hoping it's useful to others.
...
*** COMMENT Sales and CRM settings for ~org-capture~
:PROPERTIES:
...
*** My personal settings for Emacs-based presentations (~org-tree-slide~)
:PROPERTIES:
:CUSTOM_ID: h:B6449CD3-2D29-46EB-8366-7E4E70EA71CE
:CREATED:  [2025-05-02 Fri 21:16]
:END:
#+begin_src emacs-lisp :tangle "custom-lisp/vedang-personal.el"
...
:PROPERTIES:
:CUSTOM_ID: h:413A6D1D-2630-4D9E-A568-0A30C65FF26C
:CREATED:  [2025-06-08 Sun 15:46]
:END:
